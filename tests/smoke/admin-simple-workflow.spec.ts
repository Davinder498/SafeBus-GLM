...
async function mockAdmin(page: Page, role: 'tenant_admin' | 'guardian' = 'tenant_admin') {
  ...
  await page.route('**/*', async (route: Route) => {
    ...
    if (path.includes('/rpc/get_admin_trip_overview')) return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify([{ trip_id: ids.trip, service_date: '2026-01-01', status: 'active', started_at: '2026-01-01T08:00:00Z', ended_at: null, route_name: 'Route One', route_code: 'R1', trip_pattern_name: 'Morning service', direction: 'forward', bus_label: 'Bus One', driver_label: 'Test Driver' }]) });
