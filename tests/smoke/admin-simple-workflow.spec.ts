...
test.describe('Phase 12 simple admin workflow', () => {
  ...
  test('setup shows progress, missing prerequisites, and clear workflow actions', async ({ page }) => { await mockAdmin(page); await page.goto('/admin/setup'); await expect(page.getByRole('heading', { name: 'Transportation setup', level: 1 })).toBeVisible(); await expect(page.getByTestId('admin-setup-progress')).toContainText('0 of 9 steps complete'); await expect(page.getByTestId('admin-setup-progress')).toContainText('Next step: Buses'); await expect(page.getByRole('heading', { name: 'Student routes and stops', exact: true })).toBeVisible(); });
