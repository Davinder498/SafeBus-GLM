...
interface Props {
  onConfirm: (studentId: string, action: 'pickup' | 'dropoff') => Promise<void>;
...
export function StudentQrScanner({ onConfirm, busyStudentId }: Props) {
  ...
  async function processToken(token: string) {
    ...
    stopCamera();
    try {
      setResult(await resolveStudentQrForActiveTrip(token));
      setState('resolved');
    } catch {
      setState('invalid');
    }
  ...
  const nextAction =
    result?.nextEventType === 'picked_up'
      ? 'pickup'
      : result?.nextEventType === 'dropped_off'
        ? 'dropoff'
        : null;
  ...
  return (
    <Card className="p-5" data-testid="driver-qr-scanner-card">
      <div className="space-y-4">
        <div>
          <h2 className="text-lg font-bold text-navy-900">Scan student QR badge</h2>
          <p className="text-sm text-gray-600">
            Scanning identifies a badge only. The active trip and event rules are verified
            server-side.
          </p>
        </div>
        {!open && (
          <Button type="button" onClick={() => void start()} data-testid="driver-open-qr-scanner">
            Open scanner
          </Button>
        )}
        {open && (
          <div className="space-y-3">
            ...
            {state === 'scanning' && (
              <p className="text-sm text-gray-600">
                Point the rear camera at the SafeBus QR badge.
              </p>
            )}
            ...
            {state === 'unsupported' && (
              <p className="text-sm font-semibold text-warning-700">
                This browser cannot scan QR codes here. Use HTTPS and a browser with BarcodeDetector
                support.
              </p>
            )}
            {state === 'invalid' && (
              <p className="text-sm font-semibold text-danger-700">
                Badge could not be verified for this active trip.
              </p>
            )}
            ...
            {result && (
              <div
                ...
                data-testid="driver-qr-confirmation"
              >
                ...
                {nextAction ? (
                  <Button
                    ...
                    onClick={() => onConfirm(result.studentId, nextAction)}
                    data-testid="driver-qr-confirm-event"
                  >
                    Confirm {nextAction === 'pickup' ? 'pickup' : 'drop-off'}
                  </Button>
                ) : (
                  <p className="mt-3 text-sm font-semibold text-success-700">
                    Trip events complete.
                  </p>
                )}
              </div>
            )}
