...
export function StudentBusAssignmentForm({
  assignment,
  studentLabel,
  fixedStudentId,
  services,
  ...
  async function submit(event: FormEvent) {
    ...
    setError(null);
    if (!studentId || !service || !defaultTenantId) {
      setError('Choose a student and bus service.');
      return;
    }
    if (pickupStopId && !availableStops.some((stop) => stop.id === pickupStopId)) {
      setError('Pickup stop must belong to the bus service route.');
      return;
    }
    if (dropoffStopId && !availableStops.some((stop) => stop.id === dropoffStopId)) {
      setError('Drop-off stop must belong to the bus service route.');
      return;
    }
    ...
    const values = {
      ...
      bus_route_assignment_id: service.id,
      route_trip_pattern_id: service.route_trip_pattern_id,
    ...
    setSaving(true);
    try {
      await onSubmit(assignment ? values : { tenant_id: defaultTenantId, ...values });
    } catch (submitError) {
      setError(
        submitError instanceof Error
          ? submitError.message
          : 'Unable to save this student assignment.',
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <form className="grid gap-4" onSubmit={submit}>
      ...
      <label className="text-sm font-semibold text-gray-700">
        ...
        >
          <option value="">Choose bus and route</option>
          {services.map((item) => (
            <option key={item.id} value={item.id}>
              Bus {item.bus_number} - {item.route_code} {item.route_name} (
              {item.trip_name ?? item.trip_type})
            </option>
          ))}
        </select>
      </label>
        ...
        {showStatusField && (
          <label className="text-sm font-semibold text-gray-700">
            ...
            <select
              className={field}
              value={status}
              onChange={(event) => setStatus(event.target.value as StudentBusAssignmentStatus)}
            >
