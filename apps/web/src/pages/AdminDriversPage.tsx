...
import { safeBusWorkspaceReturn } from '@/utils/busWorkspace';
...
type DriverInviteFieldErrors = Partial<
  Record<Extract<DuplicateField, 'email' | 'phone' | 'licenseNumber'>, string>
>;
...
export function AdminDriversPage() {
  ...
  async function inviteDriver(event: FormEvent) {
    ...
    setMessage(null);
    try {
      const result = await inviteTenantMember({ role: 'driver', ...inviteForm });
      setInviteForm(emptyInvite);
      setShowInviteForm(false);
      setMessage(
        'Driver invitation sent. The email link lets them activate their account securely.',
      );
      await list.reload();
      const returnTo = safeBusWorkspaceReturn(searchParams.get('returnTo'), window.location.origin);
      if (returnTo && result.driverId) {
        const url = new URL(returnTo, window.location.origin);
        url.searchParams.set('newDriverId', result.driverId);
        navigate(`${url.pathname}${url.search}`, { replace: true });
      }
    } catch (error) {
      if (
        error instanceof DuplicateIdentifierError &&
        ['email', 'phone', 'licenseNumber'].includes(error.field)
      ) {
  ...
  return (
    ...
    >
      <div className="space-y-6">
          ...
          <Card className="p-5">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">
                The email becomes the login. Licence and address details stay tenant-scoped behind
                RLS.
              </p>
            </div>
            <form className="mt-5 space-y-5" onSubmit={(event) => void inviteDriver(event)}>
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <Field label="First name" htmlFor="driver-first-name" required>
                  <Input
                ...
                <Field label="Last name" htmlFor="driver-last-name" required>
                  <Input
                    id="driver-last-name"
                    maxLength={100}
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                  ...
                  >
                    <Input
                      ...
                      onChange={(event) => {
                        setInviteForm({ ...inviteForm, licenseNumber: event.target.value });
                        setFieldErrors((current) => ({ ...current, licenseNumber: undefined }));
                      }}
                    />
                  </Field>
                  <Field label="Issue date" htmlFor="driver-licence-issue" required>
                    <Input
                      id="driver-licence-issue"
                      type="date"
                      required
                      value={inviteForm.licenseIssueDate}
                      onChange={(event) =>
                        setInviteForm({ ...inviteForm, licenseIssueDate: event.target.value })
                      }
                    />
                  </Field>
                  <Field label="Expiry date" htmlFor="driver-licence-expiry" required>
                    <Input
                      id="driver-licence-expiry"
                      type="date"
                      min={inviteForm.licenseIssueDate || undefined}
                      required
                      value={inviteForm.licenseExpiryDate}
                      onChange={(event) =>
                        setInviteForm({ ...inviteForm, licenseExpiryDate: event.target.value })
                      }
                    />
                  </Field>
                  <Field label="Licence class" htmlFor="driver-licence-class" required>
                    <Select
                      id="driver-licence-class"
                      required
                      value={inviteForm.licenseClass}
                      onChange={(event) =>
                        setInviteForm({ ...inviteForm, licenseClass: event.target.value })
                      }
                    >
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                  ...
                  >
                    <Input
                      ...
                      onChange={(event) =>
                        setInviteForm({ ...inviteForm, addressLine1: event.target.value })
                      }
                    />
                  </Field>
                  <Field
                    label="Address line 2"
                    htmlFor="driver-address-2"
                    hint="Optional"
                    className="lg:col-span-2"
                  >
                    <Input
                      ...
                      onChange={(event) =>
                        setInviteForm({ ...inviteForm, addressLine2: event.target.value })
                      }
                    />
                  </Field>
                  <Field label="City" htmlFor="driver-city" required>
                    <Input
                      id="driver-city"
                      maxLength={100}
                      ...
                      onChange={(event) =>
                        setInviteForm({ ...inviteForm, city: event.target.value })
                      }
                    />
                  </Field>
                  <Field label="Province" htmlFor="driver-province" required>
                    <Input
                      id="driver-province"
                      maxLength={2}
                      ...
                      onChange={(event) =>
                        setInviteForm({ ...inviteForm, province: event.target.value.toUpperCase() })
                      }
                    />
                  </Field>
                  <Field label="Postal code" htmlFor="driver-postal-code" required>
                    <Input
                      id="driver-postal-code"
                      maxLength={7}
        ...
        <Field label="Search drivers" htmlFor="driver-search">
          <Input
            id="driver-search"
            type="search"
            value={list.searchInput}
            onChange={(event) => list.setSearchInput(event.target.value)}
            placeholder="Search by name, email, phone, or licence number"
          />
        </Field>
        {list.loading && (
        ...
        {!list.loading && !list.error && list.rows.length === 0 && (
          <DataState
            ...
            message="Invite a driver or try a different search."
          />
        )}
                      ...
                      >
                        ...
                        <td className="overflow-hidden px-5 py-3">
                          <span className="flex min-w-0 items-center gap-2 text-slate-600">
                            ...
                            <span className="truncate" title={driver.email}>
                              {driver.email}
                            </span>
                          </span>
                        </td>
                        <td className="overflow-hidden px-5 py-3">
                          <span className="flex min-w-0 items-center gap-2 text-slate-600">
                            ...
                            <span className="truncate" title={driver.phone ?? 'Not provided'}>
                        ...
                        <td className="px-5 py-3">
                          <span
                            className={`flex items-center gap-2 text-sm ${needsAttention ? 'text-warning-700' : 'text-slate-600'}`}
                          >
                            {needsAttention ? (
