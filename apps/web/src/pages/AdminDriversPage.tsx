                ...
                <tbody className="divide-y divide-slate-100">
                  {list.rows.map((driver) => {
                    ...
                    return <tr key={driver.id} className="h-16 whitespace-nowrap transition-colors hover:bg-slate-50/70">
                      <td className="overflow-hidden px-5 py-3"><div className="flex min-w-0 items-center gap-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-navy-50 text-navy-700"><UserRound className="h-4 w-4" aria-hidden /></span><span className="truncate font-bold text-navy-900">{driverName(driver)}</span></div></td>
                      <td className="overflow-hidden px-5 py-3"><span className="flex min-w-0 items-center gap-2 text-slate-600"><Mail className="h-4 w-4 shrink-0 text-slate-400" aria-hidden /><span className="truncate">{driver.email}</span></span></td>
                      <td className="overflow-hidden px-5 py-3"><span className="flex min-w-0 items-center gap-2 text-slate-600"><Phone className="h-4 w-4 shrink-0 text-slate-400" aria-hidden /><span className="truncate">{driver.phone ?? 'Not provided'}</span></span></td>
                      ...
                      <td className="px-5 py-3 text-right"><Link to={`/admin/drivers/${driver.id}`} aria-label={`View ${driverName(driver)}`} className="inline-flex h-9 items-center gap-2 rounded-lg px-3 text-sm font-semibold text-navy-700 transition-colors hover:bg-navy-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-navy-500"><Eye className="h-4 w-4" aria-hidden />View</Link></td>
                    </tr>;
                  })}
                </tbody>
              </table>
