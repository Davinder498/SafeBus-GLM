...
type State = { trips: DriverTrip[]; assignments: DriverRouteAssignment[]; buses: Bus[]; drivers: Driver[]; routes: Route[]; profiles: OrganizationProfile[] };
type TripNames = { bus: Map<string, string>; route: Map<string, string>; driver: Map<string, string> } | null;

export function AdminTripsPage() {
  const [state, setState] = useState<State | null>(null);
  const [error, setError] = useState(false);
  useEffect(() => { void Promise.all([fetchAdminTrips(), fetchAdminAssignments(), getVisibleBuses(), getVisibleDrivers(), getVisibleRoutes(), getVisibleProfiles()]).then(([trips, assignments, buses, drivers, routes, profiles]) => setState({ trips, assignments, buses, drivers, routes, profiles })).catch(() => setError(true)); }, []);
  const names = useMemo(() => {
    if (!state) return null;
    const profile = new Map(state.profiles.map((item) => [item.id, item.full_name]));
    return { bus: new Map(state.buses.map((item) => [item.id, `Bus ${item.bus_number}`])), route: new Map(state.routes.map((item) => [item.id, item.route_name])), driver: new Map(state.drivers.map((item) => [item.id, profile.get(item.profile_id) ?? 'Driver'])) };
  }, [state]);
