...
export function NotificationSettingsPage() {
  ...
  async function save(){if(!value)return;setSaving(true);setMessage(null);try{let next:NotificationPreferences=value;if(next.pushEnabled&&window.SafeBusNativePush){const permission=await window.SafeBusNativePush.enable();if(permission!=='granted')next={...next,pushEnabled:false};}else if(!next.pushEnabled){await window.SafeBusNativePush?.deactivate();}setValue(await saveNotificationPreferences(next));setMessage('Notification settings saved.');}catch(e){setMessage(e instanceof Error?e.message:'Could not save settings.');}finally{setSaving(false);}}
  return <DashboardLayout title="Notification settings" portal={portal} navItems={[]} navGroups={nav}><PageHeader title="Notification settings" description="In-app notifications always remain in your authorized inbox. These choices control Android push only."/>
    <form className="space-y-5" onSubmit={(e)=>{e.preventDefault();void save();}}><Card className="space-y-4 p-5">
      {isAdmin?<p className="rounded-lg bg-slate-100 p-3 text-sm text-slate-700">Administrators receive web inbox notifications only.</p>:<div><label className="flex items-start gap-3"><input type="checkbox" checked={value.pushEnabled} onChange={(e)=>update({pushEnabled:e.target.checked})}/><span><b>Android push notifications</b><span className="block text-sm text-slate-600">Off by default. Android will ask for permission after you save and enable this choice.</span></span></label>{window.SafeBusNativePush?<Button type="button" variant="ghost" className="mt-2" onClick={()=>void window.SafeBusNativePush?.openSystemSettings()}>Open Android notification controls</Button>:null}</div>}
