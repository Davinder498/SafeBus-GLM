-- Correct the schema of the existing identity helpers in the planned-assignment RPC.
-- Migration 0093 referenced private helpers that were never defined, so every
-- tenant-admin save failed before validation with SQLSTATE 42883.
-- Preserve the tenant, role, dates, trip guards, history behavior, and grants.
-- Forward-only repair: do not edit or reapply migration 0093.
...
begin
  if auth.uid() is null
    ...
    or public.current_user_role() <> 'tenant_admin' then
