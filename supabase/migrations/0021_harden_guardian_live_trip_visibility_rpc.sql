-- SafeBus Alberta - harden guardian live trip visibility RPC
--
-- Milestone 6A security hardening: redefines the guardian live trip visibility RPC.
--
-- This corrective migration is ADDITIVE ONLY. It uses CREATE OR REPLACE
-- FUNCTION for public.get_guardian_live_trip_visibility() because 0020 may
-- already have been applied to hosted Supabase DEV.
--
...
as $$
  ...
  -- SECURITY DEFINER hardening: stop rows must belong to the same tenant
  -- and the same route as the active student route assignment. Active-only
  -- avoids surfacing retired/archived stop names through malformed data.
  ...
  left join lateral (
    select dt.id, dt.status, dt.started_at, dt.bus_id, dt.route_id, dt.driver_id
  ...
  -- SECURITY DEFINER hardening: current-location rows must match the active
  -- trip, caller tenant, assigned route, trip bus, and trip driver.
  left join public.driver_trip_current_locations loc
    ...
    and loc.bus_id = t.bus_id
    and loc.driver_id = t.driver_id
...
comment on function public.get_guardian_live_trip_visibility() is
  ...
  'SECURITY DEFINER hardening requires same-tenant/same-route stops, same-tenant '
  'active bus/driver for the trip, and current-location route/bus/driver values '
  'matching the selected active trip — no history trail, no bus/driver ids, no '
  'speed, no trip uuid. Completed/cancelled trips are never shown as active. '
  'Drivers, admins, and other roles get zero rows.';
