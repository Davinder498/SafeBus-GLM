...
through `0096` are recorded or applied in canonical order.
...
4. Treat every migration after `0088` as pending until the protected adoption or
   release ledger proves otherwise. Never mark `0089`–`0096` applied from observed
...
`0001` through `0096`, excluding `supabase/legacy`, and retain:
